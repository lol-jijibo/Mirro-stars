...
from app.models.schemas import AnswerResponse, QuestionResponse, AnswerWithQuestion, Source, SolutionStep, ActionSummary
...
async def get_answer_by_id(answer_id: str) -> AnswerWithQuestion:
    ...
    async with get_db() as db:
        ...
        answer_rows = await db.execute_fetchall(
            """SELECT id, question_id, content, answer_type, flowchart_mermaid, steps, sources, action_summary, related_questions, created_at
        ...
        action_summary_data = json.loads(row[7]) if len(row) > 7 and row[7] else None
        related_questions_data = json.loads(row[8]) if len(row) > 8 and row[8] else []
